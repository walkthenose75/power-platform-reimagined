...
| Acceptance says **Code app in solution: none** | App built but not added to the solution | `scripts/add-app-to-solution.ps1 -EnvironmentUrl <url> -SolutionUnique <name> -AppName "<app display name>"` (or `-AppId <push‑URL id>`) — it resolves the real `canvasappid` (code app = `canvasapptype 4`, type 300), retries, adds, verifies. Portal *Add existing → App* is the fallback. |
